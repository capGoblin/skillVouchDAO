...
contract UserRequestContract {
    ...
    function createRequest(string memory _skill, string memory _project, string memory _experience, uint256 _stakeAmount) external payable {
        require(_stakeAmount > 20, "Stake amount must be greater than 20");

        uint256 reqId = userRequestStruct.add(msg.sender, new address[](0), 0, _skill, _project, _experience, _stakeAmount, block.timestamp);
